...
(async () => {
  ...
  */
  ...
  async function loadMyDictionary() {
    const filePath = "./testDictionary.json";

    try {
      // Await pauses the execution of this async function until the Promise resolves
      // Pass encoding in an options object for fs.promises.readFile
      const data = await fs.readFile(filePath, { encoding: "utf8" });
      const results = JSON.parse(data);
      console.log("Dictionary loaded successfully:"); // Log the loaded data
      return results; // Return the loaded dictionary
    } catch (error) {
  ...
  results = await loadMyDictionary();
  results["allErrors"] = results["allErrors"] ? results["allErrors"] : [];
  console.log(`debug: lengthResults: ${results.length}`);

  console.log("About to process external links...");
  // Process links to externalURLs.
  const errorsFromExternalUrlLinks = await processExternalUrlLinks(results);
  console.log("ENDED process external links...");
